"""
Modul de licențiere pentru integrarea Fleet.
...
class LicenseManager:
    """Gestionează licența pentru integrarea Fleet.
    ...
    async def async_load(self) -> None:
        ...
        _LOGGER.debug("[Fleet:License] Încep async_load()")
        try:
            stored = await self._store.async_load()
            self._data = dict(stored) if stored else {}
        except Exception:  # noqa: BLE001
            _LOGGER.warning(
                "[Fleet:License] Storage corupt sau ilizibil "
                "— pornesc cu date goale (serverul va restaura starea)"
            )
            self._data = {}
        _LOGGER.debug(
            "[Fleet:License] Date din storage: %d chei (%s)",
            len(self._data),
        ...
        _LOGGER.debug(
            "[Fleet:License] Fingerprint generat: %s... (hw: %s...)",
        ...
        if self._status_token:
            ...
            _LOGGER.debug(
                "[Fleet:License] Cache restaurat: status=%s, cache_valid=%s",
                cached_status,
                cache_valid,
            )
        else:
            _LOGGER.debug("[Fleet:License] Niciun cache de status — prima rulare")

        # Verifică statusul la server (prima verificare la startup)
        _LOGGER.debug("[Fleet:License] Verific statusul la server (startup)...")
        await self.async_check_status()

        self._loaded = True
        final_status = self.status
        _LOGGER.debug(
            "[Fleet:License] async_load() finalizat — status=%s, is_valid=%s",
            final_status,
            self.is_valid,
        )
        ...
        if final_status == "licensed":
            ...
            _LOGGER.info(
                "[Fleet:License] ✓ Licență ACTIVĂ (cheie: %s)", key
            )
        elif final_status == "trial":
            days = self.trial_days_remaining
            _LOGGER.info(
                "[Fleet:License] ⏳ Perioadă de evaluare (trial): "
                "%d zile rămase", days
            )
        elif final_status == "expired":
            _LOGGER.warning(
                "[Fleet:License] ✗ EXPIRAT — perioada de evaluare "
                "sau licența a expirat. Senzorii nu vor funcționa."
            )
        else:
            _LOGGER.warning(
                "[Fleet:License] ✗ FĂRĂ LICENȚĂ (status=%s) — "
                "senzorii nu vor funcționa.", final_status
            )

    async def _async_save(self) -> None:
        """Salvează datele de licență."""
        _LOGGER.debug("[Fleet:License] Salvez datele în storage")
    ...
    def _generate_fingerprint(self) -> str:
        ...
        - Salt: face fingerprint-ul specific integrării Fleet
        """
        componente: list[str] = []
    ...
    async def async_check_status(self) -> dict[str, Any]:
        """Verifică statusul la server (/fleet/v1/check).
        ...
        if self._is_status_cache_valid():
            _LOGGER.debug(
                "[Fleet:License] Cache valid — folosesc token existent "
        ...
        _LOGGER.debug(
            "[Fleet:License] Cache expirat sau inexistent — "
        ...
        try:
            ...
            async with session.post(
                ...
                _LOGGER.debug(
                    "[Fleet:License] Server /check răspuns: HTTP %d",
                    resp.status,
                )
                ...
                if resp.status == 200 and "status" in result:
                    ...
                    if not self._verify_token_signature(result):
                        _LOGGER.warning(
                            "[Fleet:License] Semnătura token-ului de status "
                    ...
                    if server_key and self._data.get("license_key") != server_key:
                        ...
                        _LOGGER.debug(
                            "[Fleet:License] license_key sincronizat "
                    ...
                    if client_secret and self._data.get("client_secret") != client_secret:
                        ...
                        _LOGGER.debug(
                            "[Fleet:License] client_secret sincronizat "
                            "din răspunsul /check"
                        )
                    ...
                    _LOGGER.debug(
                        "[Fleet:License] Status actualizat de la server — %s "
                        "(valid_until: %s)",
                        server_status,
                        result.get("valid_until"),
                    )

                    # Resetează flag-urile (cache e valid din nou, server accesibil)
                    self._cache_expiry_warned = False
                    ...
                    if server_status == "expired":
                        _LOGGER.warning(
                            "[Fleet:License] Server confirmă: EXPIRAT "
                            "(trial_days_remaining=0)"
                        )
                    elif server_status == "trial":
                        _LOGGER.info(
                            "[Fleet:License] Server confirmă: TRIAL "
                            "(zile rămase: %s)",
                    ...
                    if (
                        ...
                        _LOGGER.warning(
                            "[Fleet:License] Licență expirată "
                ...
                if result.get("error") == "invalid_hmac":
                    if self._data.get("client_secret") and not self._hmac_retry_done:
                        _LOGGER.warning(
                            "[Fleet:License] HMAC invalid — client_secret "
                    ...
                    _LOGGER.error(
                        "[Fleet:License] HMAC invalid (retry epuizat). "
                        "Serverul nu recunoaște acest dispozitiv."
                    )
                else:
                    _LOGGER.warning(
                        "[Fleet:License] răspuns invalid de la /check — %s",
                        result,
                    )
                return self._status_token
            ...
            _LOGGER.warning(
                "[Fleet:License] eroare de rețea la verificare status "
                "(încercare #%d) — %s",
            ...
            self._consecutive_failures += 1
            _LOGGER.warning(
                "[Fleet:License] eroare neașteptată la verificare status "
                "(încercare #%d) — %s",
                self._consecutive_failures,
                err,
            )
    ...
    def is_licensed(self) -> bool:
        ...
        if not self._verify_token_signature(token):
            _LOGGER.warning("[Fleet:License] semnătură token activare invalidă")
            return False

        # Verifică fingerprint-ul din token-ul de activare
        if token.get("fingerprint") != self._fingerprint:
            _LOGGER.warning("[Fleet:License] fingerprint token nu se potrivește")
            return False
        ...
        if self._status_token and not self._is_status_cache_valid():
            if self._is_within_grace_period():
                if not self._cache_expiry_warned:
                    ...
                    _LOGGER.warning(
                        "[Fleet:License] cache expirat — funcționare în perioadă "
                ...
                if not self._cache_expiry_warned:
                    _LOGGER.warning(
                        "[Fleet:License] cache expirat + perioadă de grație depășită "
    ...
    def license_key_masked(self) -> str | None:
        """Returnează cheia de licență mascată (ex: FLEET-XXXX-****)."""
        key = self._data.get("license_key")
    ...
    async def async_heartbeat(self) -> bool:
        ...
        try:
            ...
            async with session.post(
                ...
                _LOGGER.warning(
                    "[Fleet:License] heartbeat respins — %s",
                    result.get("error", "necunoscut"),
                )
                return False

        except Exception:  # noqa: BLE001
            _LOGGER.debug("[Fleet:License] heartbeat eșuat (rețea indisponibilă)")
            return False
    ...
    async def async_activate(self, license_key: str) -> dict[str, Any]:
        ...
        try:
            ...
            async with session.post(
                ...
                _LOGGER.debug(
                    "[Fleet:License] /activate răspuns: HTTP %d",
                    resp.status,
                )
                ...
                if result.get("success"):
                    ...
                    _LOGGER.info(
                        "[Fleet:License] licență activată cu succes (%s)",
                        token.get("license_type", "necunoscut"),
                    )

                    # Auto-reload: reîncarcă toate entry-urile fleet
                ...
                _LOGGER.warning(
                    "[Fleet:License] activare eșuată — %s (răspuns: %s)",
                    error,
                    result,
                )
            ...
            _LOGGER.error(
                "[Fleet:License] eroare de rețea la activare — %s", err
            )
            return {"success": False, "error": "network_error"}
        except Exception as err:  # noqa: BLE001
            _LOGGER.error(
                "[Fleet:License] eroare neașteptată la activare — %s", err
            )
        ...
        try:
            ...
            async with session.post(
                ...
                if resp.status == 200 and result.get("success"):
                    ...
                    _LOGGER.info(
                        "[Fleet:License] licență dezactivată cu succes"
                    )
            ...
            _LOGGER.error("[Fleet:License] eroare la dezactivare — %s", err)
            return {"success": False, "error": "network_error"}
            ...
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    if not result.get("success"):
                        _LOGGER.warning(
                            "[Fleet:License] Server a refuzat '%s': %s",
                            action, result.get("error"),
                        )
                else:
                    _LOGGER.warning(
                        "[Fleet:License] Notify HTTP %d pentru '%s'",
                        resp.status, action,
                    )
        except Exception as err:  # noqa: BLE001
            # Fire-and-forget: nu blocăm unload-ul dacă rețeaua e indisponibilă
            _LOGGER.debug(
                "[Fleet:License] Nu s-a putut raporta '%s': %s",
                action, err,
            )
    ...
    async def _async_reload_entries(self) -> None:
        """Reîncarcă toate entry-urile fleet după activare/dezactivare.
        ...
        _LOGGER.info(
            "[Fleet:License] Reîncarc %d entry-uri după schimbarea licenței",
            len(entries),
        )
    ...
    def _verify_token_signature(self, token: dict[str, Any]) -> bool:
        ...
        try:
            ...
            _LOGGER.debug("[Fleet:License] nicio cheie publică nu a validat semnătura")
            return False

        except Exception as err:  # noqa: BLE001
            _LOGGER.debug(
                "[Fleet:License] verificare semnătură eșuată — %s", err
            )
            return False
